...
export const addProducts = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const amount = parseFloat(
            mappedProducts.reduce((total: number, product: any) => total + (product.productrate * product.productquantity), 0).toFixed(2)
        );
        ...
        res.status(201).json({ message: 'Invoice created successfully', invoiceId: newInvoice._id });
